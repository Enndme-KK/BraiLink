<template>
	<view class="page">
			...
			<view class="hero-inner">
				<view class="user-row ds-rise ds-d1">
					<view class="avatar-box">
						<image :src="patient.img_url" class="avatar-img" mode="aspectFill"></image>
						<view class="avatar-tag"><text class="avatar-tag-text">患者</text></view>
					</view>
					<view class="user-info">
						<view class="name-row">
							<text class="user-name" v-if="name_display">{{ patient.name }}</text>
							<text class="user-name" v-else>*{{ patient.name.substring(1) }}</text>
							<view class="eye-btn" @click="change_eye_status">
								<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="rgba(255,255,255,0.85)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
									...
									<path v-else d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-7-11-7a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A10.07 10.07 0 0 1 12 4c7 0 11 7 11 7a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/>
									<path v-else d="M1 1l22 22"/>
								</svg>
							</view>
						</view>
						<view class="id-row">
							<text class="user-id ds-mono" v-if="id_card_display">{{ patient.card_id }}</text>
							<text class="user-id ds-mono" v-else>***********{{ patient.card_id ? patient.card_id.substring(14) : '' }}</text>
						...
						<view class="user-tags">
							<view class="tag-pill"><text class="tag-text">{{ patient.gender }}</text></view>
							<view class="tag-pill"><text class="tag-text">{{ patient.age }}岁</text></view>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="list-group ds-rise ds-d2">
				<view class="list-item" v-for="(setting, index) in setting_list" :key="index" @click="toPage(index)" hover-class="item-pressed">
					<view class="item-ico" :style="{ background: getIconBg(index) }">
						<view v-html="getSettingSvg(index)" class="ico-svg"></view>
					</view>
					<view class="item-body">
						<text class="item-title">{{ setting }}</text>
						<text class="item-desc">{{ getSettingDesc(index) }}</text>
					</view>
					<view class="item-badge" v-if="index === 4 && unreadNotificationCount > 0">
		...
		<bottom-nav-patient current="profile"></bottom-nav-patient>
	</view>
</template>

<script>
import BottomNavPatient from '@/components/bottom_nav/BottomNavPatient.vue'
import { patientAPI, notificationAPI } from '@/utils/request.js'
...
export default {
	...
	data() {
		return {
			patient: { name: '加载中...', gender: 'M', card_id: '', age: 0, img_url: '../../static/resource/boy.png' },
			setting_list: ['完善档案', '家属绑定', '账号管理', '修改密码', '意见反馈', '关于我们'],
			name_display: false, id_card_display: false, loading: true, unreadNotificationCount: 0
		}
	},
	onLoad() { this.loadPatientInfo(); this.loadUnreadNotificationCount() },
	...
	methods: {
		async loadPatientInfo() {
			try {
				...
				if (r) this.patient = {
					name: r.name || '未知',
					gender: r.gender === 'M' ? '男' : '女',
					card_id: r.id_card || '',
					age: r.age || 0,
					img_url: r.gender === 'M' ? '../../static/resource/boy.png' : '../../static/resource/girl.png'
				}
			} catch (error) {
				if (error.message?.includes('未找到') || error.message?.includes('404')) {
					uni.showModal({
						title: '提示', content: '您尚未完善患者档案，是否现在去完善？',
						confirmText: '去完善', cancelText: '稍后',
						success: r => { if (r.confirm) uni.navigateTo({ url: '/pages/completeProfile/completeProfile?userType=patient&isNew=true' }) }
					})
				} else uni.showToast({ title: '加载信息失败', icon: 'none' })
		...
		toPage(index) {
			const pages = [
				'/pages/completeProfile/completeProfile?userType=patient',
				'/pages/patient/bind-family',
		...
		getSettingSvg(i) { return [SETTING_SVG.profile, SETTING_SVG.bind, SETTING_SVG.account, SETTING_SVG.pwd, SETTING_SVG.feedback, SETTING_SVG.about][i] || SETTING_SVG.about },
		getSettingDesc(i) { return ['完善患者档案信息','邀请家属绑定查看病历','管理个人信息','修改登录密码','提交您的建议','软件版本信息'][i] || '' },
		getIconBg(i) {
			const list = [
...
.list-item { display: flex; align-items: center; padding: 14px 16px; gap: 14px; transition: background 0.15s; position: relative; }
...
.ico-svg { display: flex; align-items: center; justify-content: center; }
